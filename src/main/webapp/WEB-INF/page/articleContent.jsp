...
<div id="app" v-cloak>
    ...
    <div id="main">
        <el-card class="article-all">
            <div class="article-header" style="border-bottom: 1px solid #E7ECF3;">
                <h1>{{articleInfo.articleName}}</h1>
                <span class="time">{{articleInfo.createTime}}&nbsp;&nbsp;&nbsp;&nbsp;阅读：{{articleInfo.count}}</span>
                <el-tooltip class="item" effect="dark" :content="collectItem"  placement="right">
                    <el-button class="collection-button"  icon="el-icon-star-off"  type="text" @click="doCollection">{{articleCollection?'已收藏':'收藏'}}</el-button>
                </el-tooltip>
            </div>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            getArticle:function(){
                ...
                axios.get(this.contextPath+"api/article/queryById/"+articleId).then(function(res){
                    self.articleInfo=res.data;
            ...
            setItem:function(){
              var self=this;
              if(self.articleCollection){
                  self.collectItem='点击取消收藏'
              }else{
                  self.collectItem='点击收藏文章'
              }
            },
            isCollection:function(){
                ...
                var param={
                    ...
                    resourceId:self.articleInfo.articleId,
                    resourceType:'2'
                }
                axios.post(this.contextPath+"api/collection/isCollection",param).then(function(res){
                    if(res.data=='success'){
                        self.articleCollection=true;
                    }
                    if(res.data=='fail'){
                        self.articleCollection=false;
                    }
                    self.setItem();
                });
            },
            ...
            insertComment:function(){
                ...
                if(!self.userInfo.userId){
                    ...
                    self.form.resourceId=self.articleInfo.articleId;
                    self.form.resourceType='2';
            ...
            getCourseTeacher:function(){
                var self=this;;
                axios.get(this.contextPath+"api/user/queryById/"+self.articleInfo.createUser).then(function(res){
                    self.articleUserInfo=res.data;
                });
            },
